...
print("--------------------------------------------------")
print("🚀 Heartbeat: Final Recursive Recovery Engine v15.")
print("--------------------------------------------------")
...
def run_pro_engine():
    ...
    while current_solve_start <= full_end:
        ...
        # 🎯 STRATEGY: AUTOMATED DISCOVERY LOOP
        # This catches MissingDataError, extracts the var name, and retries.
        while not window_passed and attempts < 100:
            try:
                current_df = pd.concat([df, pd.DataFrame(missing_registry, index=df.index)], axis=1)
                
                # Use start=end for the 1989Q4 bridge to prevent Jacobian singular matrix errors
                ...
                # Extract variable name from: "The variable `xyz` appears in the model but has no..."
                match = re.search(r'`([^`]+)`', str(e))
                if match:
                    missing_var = match.group(1).lower()
                    print(f"🛡️ Discovery: Adding missing series `{missing_var}`")
                    missing_registry[missing_var] = 0.0
                    attempts += 1
                else:
    ...
    print("📈 Generating full-sample artifact...")
    model.init_trac(first_actual, full_end, final_data).to_csv(output_path)
    print(f"✅ SUCCESS: Build #522 complete.")
